...
describe("tyc Contract Tests", () => {
  ...
  it("allows another registered user to join a game and updates user stats", () => {
    ...
    simnet.callPublicFn("tyc", "register", [Cl.stringAscii("AliceUser")], alice);
    simnet.callPublicFn("tyc", "register", [Cl.stringAscii("BobUser")], bob);
    ...
    let { result: aliceUserBefore } = simnet.callReadOnlyFn("tyc", "get-user", [Cl.principal(alice)], alice);
    ...
    const { result: bobUserAfter } = simnet.callReadOnlyFn("tyc", "get-user", [Cl.principal(bob)], bob);
    ...
    const { result: gameAfter } = simnet.callReadOnlyFn("tyc", "get-game", [Cl.uint(0)], alice);
  ...
  it("allows a player to update position and balance during ongoing game", () => {
    ...
    simnet.callPublicFn("tyc", "join-game", [Cl.uint(0), Cl.uint(2)], bob);
  ...
  it("allows finalizing a game with reward and updates winner stats", () => {
    ...
    let { result: gameBefore } = simnet.callReadOnlyFn("tyc", "get-game", [Cl.uint(0)], alice);
    ...
    // Alice stats updated
    const { result: aliceUserAfter } = simnet.callReadOnlyFn("tyc", "get-user", [Cl.principal(alice)], alice);
  ...
  it("finalizes game without bonus if turns < MIN", () => {
    // Setup similar
    simnet.callPublicFn("tyc", "register", [Cl.stringAscii("CharlieUser")], charlie);
    simnet.callPublicFn("tyc", "register", [Cl.stringAscii("DaveUser")], dave);
    ...
    simnet.callPublicFn("tyc", "join-game", [Cl.uint(0), Cl.uint(2)], dave);
    ...
    const { result: charlieUserAfter } = simnet.callReadOnlyFn("tyc", "get-user", [Cl.principal(charlie)], charlie);
  ...
  it("allows a player to be removed and ends game if one remains", () => {
    ...
    // Bob joins
    const { result: joinRes } = simnet.callPublicFn("tyc", "join-game", [Cl.uint(0), Cl.uint(2)], bob);
    ...
    // Read the game back
    const { result: endedGame } = simnet.callReadOnlyFn("tyc", "get-game", [Cl.uint(0)], alice);
